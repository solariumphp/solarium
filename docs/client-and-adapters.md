...
`CurlAdapter` also implements `ConnectionTimeoutAwareInterface` and expose a `setConnectionTimeout` method to set a different value for the connection timeout, otherwise the timeout value is used 
